    ...
    try:
        with socket.create_connection(
            ...
            if ZABBIX_TLS_CONNECT == "psk":
                if not sslpsk3:
                    log.error("ZABBIX_TLS_CONNECT configurado para 'psk' mas o modulo 'sslpsk3' nao esta instalado.")
                    return
                if not ZABBIX_TLS_PSK_IDENTITY or not ZABBIX_TLS_PSK:
                    log.error("ZABBIX_TLS_PSK_IDENTITY e ZABBIX_TLS_PSK devem ser fornecidos para usar PSK.")
                    return
                try:
                    psk_bytes = bytes.fromhex(ZABBIX_TLS_PSK)
                except ValueError:
                    log.error("ZABBIX_TLS_PSK deve ser uma string hexadecimal valida.")
                    return
                ...
                with sslpsk3.wrap_socket(
                    ...
                    log.debug("Batch enviado (%d itens) [TLS/PSK] | Resposta: %s", len(items), response)
            else:
                sock.sendall(packet)
                response = sock.recv(4096)
                log.debug("Batch enviado (%d itens) [PLAIN] | Resposta: %s", len(items), response)
